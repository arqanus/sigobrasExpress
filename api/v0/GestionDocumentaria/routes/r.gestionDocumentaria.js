...
module.exports = (app) => {
  app.post("/gestiondocumentaria_mensajes", async (req, res) => {
    try {
      var message = "";
      var response = await User.postgestiondocumentaria_mensajes(
        req.body.mensaje
      );
      if (response.affectedRows > 0) {
        ...
        var response2 = await User.postgestiondocumentaria_receptores(
          req.body.receptores
        );
        ...
        console.log("receptores", req.body.receptores);
        var usuarioData = await Reusable.getDatosUsuario(
          req.body.mensaje.emisor_id
        );
        //ingreso a notificaciones
        req.body.receptores.forEach(async (item) => {
          var id_ficha = item[1];
          var fichasAccesos = await Notificacion.getFichasHasAccesos({
            id_ficha,
          });
          var dataTemp = [];
  ...
  app.post("/gestiondocumentaria_mensajes_archivoAdjunto", async (req, res) => {
    try {
      ...
      if (formFiles.files.archivoAdjunto) {
        ...
        var archivo_name =
          "gdmi_" +
          formFiles.fields.gestiondocumentaria_mensajes_id +
          "-" +
          formFiles.files.archivoAdjunto.name;
        ...
        fs.rename(
          formFiles.files.archivoAdjunto.path,
          dir + obraFolder,
          (err) => {}
        );
      ...
      var request = {
        ...
        gestiondocumentaria_mensajes_id:
          formFiles.fields.gestiondocumentaria_mensajes_id,
        gestiondocumentaria_archivosadjuntos_tipos_id:
          formFiles.fields.tipoDocumento_id,
      };
      console.log("request", request);
      var response = await User.postgestiondocumentaria_mensajes_archivoAdjunto(
        request
      );
      res.json("exito");
    } catch (error) {
  ...
  app.post(
    "/gestiondocumentaria_mensajes_archivoAdjunto_respuesta",
    async (req, res) => {
      try {
        var dir = publicFolder;
        //crear ruta si no existe
        ...
        if (formFiles.files.archivoAdjunto) {
          ...
          var archivo_name =
            "gdmi_" +
            formFiles.fields.gestiondocumentaria_respuestas_id +
            "-" +
            formFiles.files.archivoAdjunto.name;
          //se verifica y crea las carpetas de obra
          var obraFolder =
            formFiles.fields.obra_codigo +
            `/DOCUMENTOS/${formFiles.fields.tipoDocumento}/`;
          if (!fs.existsSync(dir + obraFolder)) {
            fs.mkdirSync(dir + obraFolder, { recursive: true });
          }
          obraFolder += archivo_name;
        ...
        var request = {
          documento_link: link,
          gestiondocumentaria_respuestas_id:
            formFiles.fields.gestiondocumentaria_respuestas_id,
          gestiondocumentaria_archivosadjuntos_tipos_id:
            formFiles.fields.tipoDocumento_id,
        };
  ...
  app.get("/gestiondocumentaria_enviados_usuarios", async (req, res) => {
    try {
      var response = await User.getgestiondocumentaria_enviados_usuarios(
        req.query
      );
      res.json(response);
    } catch (error) {
  ...
  app.put("/gestiondocumentaria_receptores", async (req, res) => {
    try {
      ...
      var response2 = await User.getgestiondocumentaria_receptores_mensaje_visto(
        req.body
      );
  ...
  app.get("/gestiondocumentaria_recibidos_respuestas", async (req, res) => {
    try {
      var response = await User.getgestiondocumentaria_recibidos_respuestas(
        req.query
      );
      res.json(response);
    } catch (error) {
  ...
  app.get(
    "/gestiondocumentaria_recibidos_respuestas_cantidad",
    async (req, res) => {
      try {
        var response = await User.getgestiondocumentaria_recibidos_respuestas_cantidad(
          req.query
        );
        res.json(response);
      } catch (error) {

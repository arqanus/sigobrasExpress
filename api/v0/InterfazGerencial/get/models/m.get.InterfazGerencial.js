module.exports = {
  ...
  getSectores({ id_acceso, id_unidadEjecutora, idsectores }) {
    return new Promise((resolve, reject) => {
      ...
      if (id_unidadEjecutora != 0 && id_unidadEjecutora != undefined) {
        condiciones.push(
          `(unidadEjecutoras_id_unidadEjecutora = ${id_unidadEjecutora})`
        );
      }
  ...
  IG_AvanceFisicoProgramado_anyos(
    id_acceso,
    unidadEjecutoras_id_unidadEjecutora,
    sectores_idsectores,
    modalidad_ejecutora,
  ...
  IG_AvanceFinanciero_porcentaje(id_ficha) {
    return new Promise((resolve, reject) => {
      pool.query(
        "SELECT (SUM(monto) + cronogramamensual.avance) / fichas.g_total_presu *100 porcentaje_avance FROM fichas LEFT JOIN historialestados ON historialestados.Fichas_id_ficha = fichas.id_ficha LEFT JOIN (SELECT fichas_id_ficha, SUM(financieroEjecutado) avance FROM cronogramamensual WHERE cronogramamensual.fichas_id_ficha = ?) cronogramamensual ON cronogramamensual.fichas_id_ficha = fichas.id_ficha WHERE fichas.id_ficha = ? ",
        [id_ficha, id_ficha],
        (error, res) => {
          if (error) {
            reject(error);

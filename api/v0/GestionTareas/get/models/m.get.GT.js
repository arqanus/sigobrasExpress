...
userModel.getTareaAccesoCargo = (id_acceso) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "select cargos.nivel from accesos left join cargos on cargos.id_Cargo = accesos.Cargos_id_Cargo where accesos.id_acceso =?",
      [id_acceso],
      (err, res) => {
        if (err) {
          return reject(err);
...
userModel.getTareasProyectos = (emireceptor, id_acceso, inicio, fin) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "select proyectos.* FROM tareas LEFT JOIN proyectos ON proyectos.id_proyecto = tareas.proyectos_id_proyecto left JOIN tareas_has_accesos ON tareas_has_accesos.tareas_id_tarea = tareas.id_tarea WHERE " +
        emireceptor +
        " = ? AND ? <= avance AND avance <= ? AND (avance = 100 or tareas.fecha_final >= NOW()) group by proyectos.id_proyecto order by tareas.fecha_final asc",
      [id_acceso, inicio, fin],
      (err, res) => {
        if (err) {
          return reject(err);
  ...
  return new Promise((resolve, reject) => {
    pool.query(
      'SELECT tareas.id_tarea,proyectos.color proyecto_color, proyectos.nombre proyecto_nombre, tareas.asunto, tareas.avance porcentaje_avance, DATEDIFF(tareas.fecha_final,now()) prioridad_color,if(proyectos.nombre = "RECORDATORIO","R","T") tipo_tarea FROM tareas LEFT JOIN proyectos ON proyectos.id_proyecto = tareas.proyectos_id_proyecto LEFT JOIN tareas_has_accesos ON tareas_has_accesos.tareas_id_tarea = tareas.id_tarea WHERE ' +
        emireceptor +
        " = ? AND ? <= avance AND avance <= ? and tareas.fecha_final >= now() and receptor is null",
      [id_acceso, inicio, fin],
      (err, res) => {
        if (err) {
          return reject(err);
...
userModel.getTareasProyectosVencidas = (
  emireceptor,
  id_acceso,
  inicio,
  fin
  ...
  return new Promise((resolve, reject) => {
    pool.query(
      "select proyectos.* FROM tareas LEFT JOIN proyectos ON proyectos.id_proyecto = tareas.proyectos_id_proyecto left JOIN tareas_has_accesos ON tareas_has_accesos.tareas_id_tarea = tareas.id_tarea WHERE " +
        emireceptor +
        " = ? AND ? <= avance AND avance <= ? AND tareas.fecha_final < now() group by proyectos.id_proyecto order by tareas.fecha_final desc",
      [id_acceso, inicio, fin],
      (err, res) => {
        if (err) {
          return reject(err);
...
userModel.getTareasVencidas = (
  ...
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT tareas.id_tarea, proyectos.color proyecto_color, proyectos.nombre proyecto_nombre, tareas.asunto, tareas.avance porcentaje_avance, DATEDIFF(tareas.fecha_final, NOW()) prioridad_color, usuarios.nombre emisor_nombre, cargos.nombre emisor_cargo FROM tareas LEFT JOIN proyectos ON proyectos.id_proyecto = tareas.proyectos_id_proyecto LEFT JOIN tareas_has_accesos ON tareas_has_accesos.tareas_id_tarea = tareas.id_tarea LEFT JOIN accesos ON accesos.id_acceso = tareas.emisor LEFT JOIN usuarios ON usuarios.id_usuario = accesos.Usuarios_id_usuario LEFT JOIN cargos ON cargos.id_Cargo = accesos.Cargos_id_Cargo WHERE " +
        emireceptor +
        " = ? AND ? <= avance AND avance <= ? and proyectos.id_proyecto = ? and tareas.fecha_final < now()",
      [id_acceso, inicio, fin, id_proyecto],
      (err, res) => {
        if (err) {
          return reject(err);
  ...
  return new Promise((resolve, reject) => {
    pool.query(
      ...
      (err, res) => {
        ...
        res[0].usuario_imagen =
          res[0].usuario_imagen || default_data.user_image_default;
    ...
    pool.query(
      ...
      (err, res) => {
        ...
        for (let i = 0; i < res.length; i++) {
          ...
          subordinado.subordinado_imagen =
            subordinado.subordinado_imagen || default_data.user_image_default;
          subordinado.subordinado_imagenAlt =
            subordinado.subordinado_imagenAlt || "test";
        }
...
userModel.getTareaSubordinadosTareas = (id_acceso) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT id_proyecto,proyectos.color prioridad_color,tareas_has_accesos.tareas_id_tarea FROM tareas_has_accesos left join tareas on tareas.id_tarea = tareas_has_accesos.tareas_id_tarea left join proyectos on proyectos.id_proyecto = tareas.proyectos_id_proyecto WHERE receptor = ? and tareas.avance < 100",
      [id_acceso],
      (err, res) => {
        if (err) {
          return reject(err);
...
userModel.getTareaComentarios = (id_comentario, id_tarea) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT comentarios.mensaje, DATE_FORMAT(comentarios.fecha, '%h:%i %p') hora, DATE_FORMAT(comentarios.fecha, '%d/%c/%Y') fecha, CONCAT(usuarios.nombre,' ',usuarios.apellido_paterno) usuario, usuarios.imagen FROM comentarios left join accesos on accesos.id_acceso = comentarios.accesos_id_acceso left join usuarios on usuarios.id_usuario = accesos.Usuarios_id_usuario where id_comentario = ? or comentarios.tareas_id_tarea = ?",
      [id_comentario, id_tarea],
      (err, res) => {
        if (err) {
          return reject(err);
        }
...
userModel.getUsuarioTareasDetalles = (id_acceso) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT usuarios.nombre, usuarios.apellido_paterno, usuarios.imagen foto, cargos.nombre cargo, COALESCE(tareasAsignadas.cantidad, 0) tareasAsignadas, COALESCE(tareasPendientes.cantidad, 0) tareasPendientes, COALESCE(tareasProgreso.cantidad, 0) tareasProgreso, COALESCE(tareasConcluidas.cantidad, 0) tareasConcluidas, COALESCE(tareasVencidas.cantidad, 0) tareasVencidas, COALESCE(tareasDesdeHastaAnios.tareasDesdeHastaAnios, '') tareasDesdeHastaAnios, (COALESCE(tareasConcluidas.cantidad, 0) / (COALESCE(tareasAsignadas.cantidad, 0))) * 100 porcentajeAvance, (COALESCE(tareasConcluidas.cantidad, 0) / (COALESCE(tareasAsignadas.cantidad, 0))) * 5 estrellas FROM accesos LEFT JOIN usuarios ON usuarios.id_usuario = accesos.Usuarios_id_usuario LEFT JOIN cargos ON cargos.id_Cargo = accesos.Cargos_id_Cargo LEFT JOIN (SELECT receptor, COUNT(tareas.id_tarea) cantidad FROM tareas LEFT JOIN tareas_has_accesos ON tareas_has_accesos.tareas_id_tarea = tareas.id_tarea LEFT JOIN proyectos ON proyectos.id_proyecto = tareas.proyectos_id_proyecto WHERE avance = 100 GROUP BY receptor) tareasConcluidas ON tareasConcluidas.receptor = accesos.id_acceso LEFT JOIN (SELECT receptor, COUNT(tareas.id_tarea) cantidad FROM tareas LEFT JOIN tareas_has_accesos ON tareas_has_accesos.tareas_id_tarea = tareas.id_tarea LEFT JOIN proyectos ON proyectos.id_proyecto = tareas.proyectos_id_proyecto WHERE avance < 100 AND tareas.fecha_final < NOW() GROUP BY receptor) tareasVencidas ON tareasVencidas.receptor = accesos.id_acceso LEFT JOIN (SELECT receptor, COUNT(tareas.id_tarea) cantidad FROM tareas LEFT JOIN tareas_has_accesos ON tareas_has_accesos.tareas_id_tarea = tareas.id_tarea LEFT JOIN proyectos ON proyectos.id_proyecto = tareas.proyectos_id_proyecto GROUP BY receptor) tareasAsignadas ON tareasAsignadas.receptor = accesos.id_acceso LEFT JOIN (SELECT receptor, COUNT(tareas.id_tarea) cantidad FROM tareas LEFT JOIN tareas_has_accesos ON tareas_has_accesos.tareas_id_tarea = tareas.id_tarea LEFT JOIN proyectos ON proyectos.id_proyecto = tareas.proyectos_id_proyecto WHERE 0 < avance AND avance < 100 AND tareas.fecha_final >= NOW() GROUP BY receptor) tareasProgreso ON tareasProgreso.receptor = accesos.id_acceso LEFT JOIN (SELECT receptor, CONCAT(MIN(YEAR(fecha_final)), ' - ', MAX(YEAR(fecha_final))) tareasDesdeHastaAnios FROM tareas LEFT JOIN tareas_has_accesos ON tareas_has_accesos.tareas_id_tarea = tareas.id_tarea LEFT JOIN proyectos ON proyectos.id_proyecto = tareas.proyectos_id_proyecto GROUP BY receptor) tareasDesdeHastaAnios ON tareasDesdeHastaAnios.receptor = accesos.id_acceso LEFT JOIN (SELECT receptor, COUNT(tareas.id_tarea) cantidad FROM tareas LEFT JOIN tareas_has_accesos ON tareas_has_accesos.tareas_id_tarea = tareas.id_tarea LEFT JOIN proyectos ON proyectos.id_proyecto = tareas.proyectos_id_proyecto WHERE avance = 0 AND tareas.fecha_final >= NOW() GROUP BY receptor) tareasPendientes ON tareasPendientes.receptor = accesos.id_acceso where accesos.id_acceso=?",
      [id_acceso],
      (err, res) => {
        if (err) {
          return reject(err);
          ...
          res[0].foto = res[0].foto || default_data.user_image_default;
          return resolve(res[0]);
        }
      }
    );
...
userModel.getObrasAcargo = (id_acceso) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT id_ficha idObra,codigo codigoObra FROM fichas_has_accesos LEFT JOIN fichas ON fichas.id_ficha = fichas_has_accesos.Fichas_id_ficha where fichas_has_accesos.Accesos_id_acceso = ?",
      [id_acceso],
      (err, res) => {
        if (err) {
          return reject(err);

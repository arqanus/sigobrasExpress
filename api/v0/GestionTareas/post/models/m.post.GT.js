...
userModel.postTareaReceptores = (data) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "INSERT INTO tareas_has_accesos (tareas_id_tarea,receptor) values ? ON DUPLICATE key UPDATE receptor = VALUES(receptor)",
      [data],
      (err, res) => {
        if (err) {
          reject(err.code);
...
userModel.postTareaAvance = (avance, id_tarea) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "update tareas SET avance = ? where id_tarea = ?",
      [avance, id_tarea],
      (err, res) => {
        if (err) {
          return reject(err);

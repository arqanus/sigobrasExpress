...
module.exports = (app) => {
            ...
            fs.rename(files.archivo.path, obraFolder + ruta, async (err) => {
              try {
                ...
                for (let i = 0; i < receptor.length; i++) {
                  ...
                  receptores.push([id_tarea, Number(element)]);
                }
                if (receptor.length) {
  ...
  app.post("/postTareaAvance", async (req, res) => {
    var Ncambios = await User.postTareaAvance(
      req.body.avance,
      req.body.id_tarea
    );
    if (Ncambios == 0) {
  ...
  app.post("/postTareaReceptores", async (req, res) => {
    try {
      ...
      var subordinadosTareas = await User2.getTareaSubordinadosTareas(
        id_acceso
      );
  ...
  app.post("/postTareaComentario", async (req, res) => {
    try {
      ...
      comentario[0].imagen =
        comentario[0].imagen || default_data.user_image_default;

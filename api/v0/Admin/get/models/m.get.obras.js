...
userModel.getObras = ({ id_tipoObra, textoBuscado }) => {
  var query = `
    SELECT
        ...
        fichas.g_meta,
        id_tipoObra,
        MAX(avanceactividades.fecha) ultimo_metrado,
        DATE_FORMAT(MAX(avanceactividades.fecha), '%Y-%m-%d') ultimo_metrado
    FROM
        fichas
            LEFT JOIN
  ...
  query += " group by fichas.id_ficha ORDER BY 5 DESC ";
  // return query
...
userModel.getComponentesById = (id_ficha) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT componentes.numero, componentes.nombre, componentes.id_componente, componentes.presupuesto FROM componentes WHERE componentes.Fichas_id_ficha = ?",
      id_ficha,
      (err, res) => {
        if (err) reject(err);
        resolve(res);
...
userModel.getHistorialEstados = (id_ficha) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "select id_historialEstado,Fichas_id_ficha, DATE_FORMAT(fecha_inicial, '%Y-%m-%d') fecha_inicial, DATE_FORMAT(fecha_final, '%Y-%m-%d') fecha_final, Estados_id_Estado from historialestados where Fichas_id_ficha = ?",
      id_ficha,
      (err, res) => {
        if (err) {
          reject(err.code);
          ...
          for (let i = 0; i < res.length; i++) {
            ...
            Data.push([
              historial.id_historialEstado,
              historial.Fichas_id_ficha,
              historial.fecha_inicial,
              historial.fecha_final,
              historial.Estados_id_Estado,
            ]);
          }
          resolve(Data);
        }
...
userModel.getValGeneralTodosComponentes = (
  id_ficha,
  fecha_inicial,
  fecha_final
) => {
        ...
        if (error) {
          ...
          for (let i = 0; i < res.length; i++) {
            ...
            if (fila.tipo == "titulo") {
              ...
              fila.costo_unitario = tools.formatoSolesPresicion(
                fila.costo_unitario
              );
              fila.precio_parcial = tools.formatoSolesPresicion(
                fila.precio_parcial
              );
              fila.metrado_anterior = tools.formatoSolesPresicion(
                fila.metrado_anterior
              );
              fila.valor_anterior = tools.formatoSolesPresicion(
                fila.valor_anterior
              );
              fila.porcentaje_anterior = tools.formatoSolesPresicion(
                fila.porcentaje_anterior
              );
              fila.metrado_actual = tools.formatoSolesPresicion(
                fila.metrado_actual
              );
              fila.valor_actual = tools.formatoSolesPresicion(
                fila.valor_actual
              );
              fila.porcentaje_actual = tools.formatoSolesPresicion(
                fila.porcentaje_actual
              );
              fila.metrado_total = tools.formatoSolesPresicion(
                fila.metrado_total
              );
              fila.valor_total = tools.formatoSolesPresicion(fila.valor_total);
              fila.porcentaje_total = tools.formatoSolesPresicion(
                fila.porcentaje_total
              );
              fila.metrado_saldo = tools.formatoSolesPresicion(
                fila.metrado_saldo
              );
              fila.valor_saldo = tools.formatoSolesPresicion(fila.valor_saldo);
              fila.porcentaje_saldo = tools.formatoSolesPresicion(
                fila.porcentaje_saldo
              );
            }
          }
          ...
          resolve({
            ...
            porcentaje_anterior: tools.formatoSolesPresicion(
              (valor_anterior / precio_parcial) * 100
            ),
            porcentaje_actual: tools.formatoSolesPresicion(
              (valor_actual / precio_parcial) * 100
            ),
            porcentaje_total: tools.formatoSolesPresicion(
              (valor_total / precio_parcial) * 100
            ),
            porcentaje_saldo: tools.formatoSolesPresicion(
...
userModel.getCostosPresupuestales = (id_ficha) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT * FROM costospresupuestales WHERE fichas_id_ficha = ?",
      [id_ficha],
      (err, res) => {
        if (err) {
          reject(err.code);
...
userModel.getResoluciones = (id_ficha) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "select * from resoluciones where fichas_id_ficha=?",
      [id_ficha],
      (err, res) => {
        if (err) {
          reject(err.code);
...
userModel.updateActividadesParcial = ({ metrado, id_actividad }) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "UPDATE actividades SET parcial = ? WHERE (id_actividad = ?);",
      [metrado, id_actividad],
      (err, res) => {
        if (err) {
          reject(err.code);
        } else {
...
userModel.updatePartidasMetrado = ({ id_partida }) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "UPDATE partidas INNER JOIN (SELECT SUM(actividades.parcial) metrado_actividad, actividades.* FROM actividades WHERE actividades.Partidas_id_partida = ?) actividad ON actividad.Partidas_id_partida = partidas.id_partida SET partidas.metrado = actividad.metrado_actividad WHERE partidas.id_partida = ?;",

...
userModel.deleteEliminarHistorial = (id_historialEstado) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "DELETE FROM historialestados WHERE id_historialEstado = ? ",
      id_historialEstado,
      (error, res) => {
        if (error) {
          reject(error.code);
...
userModel.deleteResolucion = (id_resolucion) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "delete from resoluciones where id_resolucion = ?",
      [id_resolucion],
      (error, res) => {
        if (error) {
          reject(error.code);
...
userModel.deleteHistorialById = (id_AvanceActividades) => {
  return new Promise((resolve, reject) => {
    var query = `delete from avanceactividades where id_AvanceActividades = ${id_AvanceActividades}`;
    pool.query(query, (error, res) => {
      if (error) {
        reject(error.code);
      } else {

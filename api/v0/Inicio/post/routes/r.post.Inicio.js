...
module.exports = function (app) {
  app.post("/postcronogramamensual", async (req, res) => {
    try {
      ...
      if (periodoCorte) {
        valorizacionCorte = await User2.getValGeneralResumenPeriodo(
          req.body[0][0],
          periodoCorte.fecha_inicial,
          periodoCorte.fecha_final,
          false
        );
      ...
      var cronograma = await User3.getcronogramaInicio(
        corte,
        req.body[0][0],
        periodoCorte.fecha_final
      );
      ...
      cronograma.fecha_inicial = tools.fechaLargaCorta(
        new Date(periodoCorte.fecha_final)
      );
  ...
  app.put("/postFinancieroCorte", async (req, res) => {
    try {
      var data = await User.postFinancieroCorte(
        req.body.monto,
        req.body.id_historialEstado
      );
      ...
      if (periodoCorte) {
        ...
        var financiero_monto = await User3.getFinancieroMonto(
          req.body.id_ficha
        );
        corte.id_historialEstado = financiero_monto.id_historialEstado;
        corte.codigo = "C";

...
userModel.PartidasSemanal = (id_ficha, anyo, mes, semana, fechasavance) => {
  return new Promise((resolve, reject) => {
    var query =
      "SELECT id_partida, '' tipo, partidas_mo.item, partidas_mo.descripcion, partidas_mo.unidad_medida, partidas_mo.rendimiento, partidas_mo.costo_unitario,";
    for (let i = 0; i < fechasavance.length; i++) {
      const fechaobjeto = fechasavance[i];
      query +=
        "SUM(if(DATE_FORMAT(avanceactividades.fecha, '%Y-%m-%d') = '" +
        fechaobjeto.fecha +
        "', avanceactividades.valor, 0)) metrado" +
        i +
        ",";
    }
...
userModel.cantidadPersonal = (partidasSemana, cargos) => {
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < cargos.length; i++) {
      ...
      query +=
        "sum(IF(recursos.descripcion = '" +
        cargoObjeto.descripcion +
        "', recursos.cuadrilla, 0)) cargo_" +
        i +
        ",";
    }
    query = query.substring(0, query.length - 1);
    query +=
      " FROM partidas LEFT JOIN recursos ON recursos.Partidas_id_partida = partidas.id_partida WHERE partidas.id_partida in(";
    ...
    query +=
      ") AND recursos.tipo = 'Mano de Obra' group by id_partida order by id_partida";
...
userModel.putresumengastomo = (insertintogastomo) => {
  //nombra la query
  return new Promise((resolve, reject) => {
    var query =
      "INSERT INTO resumen_gasto_mo (idresumen_gasto_mo, anyo, mes, semana, cantidad_personal, costo_mo_gore, dias_trabajados, cargo, fichas_id_ficha) VALUES ? ON DUPLICATE KEY UPDATE cantidad_personal = values(cantidad_personal), costo_mo_gore = values(costo_mo_gore), dias_trabajados = values(dias_trabajados), cargo = values(cargo)";
    pool.query(query, [insertintogastomo], (error, resultado) => {
      //ejecutqa la query
      if (error) {
...
userModel.historial_gasto_mo = (anyo, mes, semana, id_ficha) => {
  //nombra la query
  return new Promise((resolve, reject) => {
    var query =
      "SELECT idresumen_gasto_mo, cantidad_personal cantidad, cargo, costo_mo_gore costogore, dias_trabajados dias FROM resumen_gasto_mo where anyo = ? and mes = ? and semana = ? and fichas_id_ficha = ?;";
    pool.query(query, [anyo, mes, semana, id_ficha], (error, resultado) => {
      //ejecutqa la query
      if (error) {
        reject(error); //reconoce el error
      } else {
...
userModel.gasto_semana_mo = (id_ficha, anyo, mes, semana) => {
  return new Promise((resolve, reject) => {
    var query =
      "SELECT sum(cantidad_personal * costo_mo_gore * dias_trabajados) gasto_semana_mo FROM resumen_gasto_mo WHERE fichas_id_ficha = ? and anyo = ? AND mes = ? AND semana = ?";
    pool.query(query, [id_ficha, anyo, mes, semana], (error, resultado) => {
      //ejecutqa la query
      if (error) {
        reject(error);
...
userModel.gastoestimadogore_mo_semana = (id_ficha, anyo, mes, semana) => {
  return new Promise((resolve, reject) => {
    var query =
      "SELECT SUM(cantidad_personal * costo_mo_gore * dias_trabajados) gasto_semana_mo, semana FROM resumen_gasto_mo WHERE fichas_id_ficha = ? AND anyo = ? AND mes = ? and semana = ? group by semana";
    pool.query(query, [id_ficha, anyo, mes, semana], (error, resultado) => {
      //ejecutqa la query
      if (error) {
        reject(error);

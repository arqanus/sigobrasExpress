module.exports = {
  getPlazos(id_ficha, nivel, idplazos = "null") {
    return new Promise((resolve, reject) => {
      var query =
        "SELECT plazos_historial.*, DATE_FORMAT(fecha_inicio, '%Y-%m-%d') fecha_inicio, DATE_FORMAT(fecha_final, '%Y-%m-%d') fecha_final FROM plazos_historial WHERE fichas_id_ficha = ? AND nivel = ? AND COALESCE(plazos_historial.idplazos, 'null') = '" +
        idplazos +
  ...
  postPlazosImagenes({ id, archivo, id_acceso }) {
    return new Promise((resolve, reject) => {
      var query = `
      UPDATE plazos_historial
        SET
            archivo = '${archivo}',
            archivo_editor = ${id_acceso}
        WHERE
            (id = ${id})
      `;

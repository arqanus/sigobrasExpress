...
userModel.getmaterialesResumenChart = (consulta, id_ficha) => {
  return new Promise(async (resolve, reject) => {
    var res = await userModel.getmaterialesResumenConsulta(
      consulta,
      id_ficha,
      null,
      true,
    ...
    for (let i = 0; i < res.length; i++) {
      ...
      series[1].data.push(
        tools.formatoPorcentaje(tipoRecurso.recurso_gasto_parcial)
      );
...
userModel.getmaterialesResumenTipos = (id_ficha) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT recursos.tipo FROM componentes left join partidas on partidas.componentes_id_componente = componentes.id_componente inner join recursos on recursos.Partidas_id_partida = partidas.id_partida WHERE componentes.fichas_id_ficha = ? GROUP BY recursos.tipo",
      [id_ficha],
      (error, res) => {
        if (error) {
          reject(error);
...
userModel.getmaterialesResumen = (consulta, id_ficha, tipo) => {
  return new Promise(async (resolve, reject) => {
    ...
    for (let i = 0; i < res.length; i++) {
      ...
      recurso.recurso_gasto_cantidad = tools.formatoSoles(
        recurso.recurso_gasto_cantidad
      );
      recurso.recurso_gasto_parcial = tools.formatoSoles(
        recurso.recurso_gasto_parcial
      );
...
userModel.getmaterialesResumenEjecucionReal = (
  id_ficha,
  tipo,
  todosRecursos = false,
  codigo = "null",
  ...
  todosTipos = true,
  agrupacion = "recursos.descripcion",
  formato = true
) => {
  return new Promise((resolve, reject) => {
    pool.query(
      ...
      (error, res) => {
        if (error) {
          ...
          if (formato) {
            for (let i = 0; i < res.length; i++) {
              const recurso = res[i];
              recurso.recurso_cantidad = tools.formatoSoles(
                recurso.recurso_cantidad
              );
              recurso.recurso_precio = tools.formatoSoles(
                recurso.recurso_precio
              );
              recurso.recurso_parcial = tools.formatoSoles(
                recurso.recurso_parcial
              );
              recurso.recurso_gasto_cantidad = tools.formatoSoles(
                recurso.recurso_gasto_cantidad
              );
...
userModel.getmaterialesResumenEjecucionRealChart = (id_ficha) => {
  ...
  return new Promise(async (resolve, reject) => {
    var res = await userModel.getmaterialesResumenEjecucionReal(
      id_ficha,
      "",
      true,
      "",
...
userModel.getmaterialesResumenEjecucionRealCodigos = (
  id_ficha,
  ...
  return new Promise((resolve, reject) => {
    pool.query(
      ...
      [id_ficha, tipoDocumentoAdquisicion],
      (error, res) => {
        if (error) {
          reject(error);
        } else {
...
userModel.getclasificadoresPesupuestarios = (
  ...
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT * FROM clasificadores_presupuestarios WHERE ? OR clasificadores_presupuestarios.clasificador = ?",
      [todos, clasificador],
      (error, res) => {
        if (error) {
          reject(error);
...
userModel.getmaterialescomponentesTipos = (id_componente) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT recursos.tipo FROM componentes left join partidas on partidas.componentes_id_componente = componentes.id_componente inner join recursos on recursos.Partidas_id_partida = partidas.id_partida WHERE componentes.id_componente = ? GROUP BY recursos.tipo",
      id_componente,
      (error, res) => {
        if (error) {
          reject(error);
...
userModel.getmaterialescomponentes = (id_ficha) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "select id_componente,numero,nombre from componentes where componentes.fichas_id_ficha = ?",
      id_ficha,
      (error, res) => {
        if (error) {
          reject(error);
      ...
      (error, res) => {
        ...
        for (let i = 0; i < res.length; i++) {
          ...
          if (fila.tipo == "titulo") {
            ...
            fila.metrados_costo_saldo = tools.formatoSoles(
              fila.metrados_costo_saldo
            );
...
userModel.getmaterialespartidaTipos = (id_partida) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT recursos.tipo FROM recursos WHERE recursos.Partidas_id_partida = ? GROUP BY recursos.tipo",
      id_partida,
      (error, res) => {
        if (error) {
          reject(error);
...
userModel.getRecursosNuevosCodigos = (
  id_ficha,
  id_tipoDocumentoAdquisicion
) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT codigo,count(recursosnuevos.id_recursoNuevo)cantidad,false bloqueado FROM recursosnuevos WHERE recursosnuevos.fichas_id_ficha = ? AND recursosnuevos.tipoDocumentoAdquisicion_id_tipoDocumentoAdquisicion = ? GROUP BY codigo",
      [id_ficha, id_tipoDocumentoAdquisicion],
      (error, res) => {
        if (error) {
          reject(error);
        } else {
...
userModel.getRecursosNuevosCodigosData = (
  id_ficha,
  codigo,
  id_tipoDocumentoAdquisicion
) => {
  return new Promise((resolve, reject) => {
    pool.query(
      ...
      [id_ficha, codigo, id_tipoDocumentoAdquisicion],
      (error, res) => {
        if (error) {
          reject(error);
        } else {
...
userModel.getResumenRecursosConteoDatos = ({
  id_ficha,
  tipo,
  texto_buscar,
}) => {
  var query =
    "SELECT COUNT(*) total FROM (SELECT recursos.* FROM componentes LEFT JOIN partidas ON partidas.componentes_id_componente = componentes.id_componente LEFT JOIN recursos ON recursos.partidas_id_partida = partidas.id_partida";
  var condiciones = [];
  if (id_ficha != 0) {
  ...
  if (texto_buscar != "") {
    condiciones.push(
      `(recursos.descripcion like \'%${texto_buscar}%\' || recursos.unidad like \'%${texto_buscar}%\')`
    );
  }
...
userModel.getResumenRecursos = ({
  ...
  var query =
    "SELECT recursos.*, SUM(IF(recursos.unidad = '%MO' OR recursos.unidad = '%PU', 0, recursos.cantidad * partidas.metrado)) recurso_cantidad FROM componentes LEFT JOIN partidas ON partidas.componentes_id_componente = componentes.id_componente LEFT JOIN recursos ON recursos.partidas_id_partida = partidas.id_partida";
  var condiciones = [];
  if (id_ficha != 0) {
    condiciones.push(`(componentes.fichas_id_ficha = ${id_ficha})`);
  }
...
userModel.getResumenRecursosNuevos = ({
  id_ficha,
  tipo,
  texto_buscar,
  inicio,
  ...
  if (texto_buscar != "") {
    condiciones.push(
      `(descripcion like \'%${texto_buscar}%\' || unidad like \'%${texto_buscar}%\')`
    );
  }
  if (condiciones.length > 0) {
...
userModel.getResumenRecursosRealesByDescripcion = ({
  id_ficha,
  descripcion,
}) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT * FROM recursos_ejecucionreal WHERE recursos_ejecucionreal.fichas_id_ficha = ? AND recursos_ejecucionreal.descripcion = ?",
      [id_ficha, descripcion],
      (error, res) => {
        if (error) {
          reject(error);
...
userModel.getTipoDocumentoAdquisicionTotal = ({
  id_ficha,
  id_tipoDocumentoAdquisicion,
}) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT COUNT(*) n_elementos FROM recursos_ejecucionreal WHERE recursos_ejecucionreal.fichas_id_ficha = ? AND id_tipoDocumentoAdquisicion = ?;",
      [id_ficha, id_tipoDocumentoAdquisicion],
      (error, res) => {
        if (error) {
          reject(error);
...
userModel.getRecursosEjecucionRealByTipoDocumentoAdquisicion = ({
  id_ficha,
  id_tipoDocumentoAdquisicion,
}) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT codigo, COUNT(codigo) n_elementos FROM recursos_ejecucionreal WHERE recursos_ejecucionreal.fichas_id_ficha = ? AND id_tipoDocumentoAdquisicion = ? GROUP BY codigo",
      [id_ficha, id_tipoDocumentoAdquisicion],
      (error, res) => {
        if (error) {
          reject(error);
...
userModel.getRecursosEjecucionRealByTipoAndCodigo = ({
  id_ficha,
  id_tipoDocumentoAdquisicion,
  codigo,
}) => {
  return new Promise((resolve, reject) => {
    pool.query(
      ...
      [id_ficha, id_ficha, id_tipoDocumentoAdquisicion, codigo],
      (error, res) => {
        if (error) {
          reject(error);
        }
...
userModel.getRecursosEjecucionRealNuevosByTipoAndCodigo = ({
  id_ficha,
  id_tipoDocumentoAdquisicion,
  codigo,
}) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT * FROM recursos_ejecucionreal WHERE recurso_nuevo AND fichas_id_ficha = ? AND id_tipoDocumentoAdquisicion = ? AND recursos_ejecucionreal.codigo = ?",
      [id_ficha, id_tipoDocumentoAdquisicion, codigo],
      (error, res) => {
        if (error) {
          reject(error);
        }
...
userModel.getDocumentoAdquisicionDetalles = ({
  id_ficha,
  id_tipoDocumentoAdquisicion,
  codigo,
}) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT * FROM documentosadquisicion WHERE fichas_id_ficha = ? AND id_tipoDocumentoAdquisicion = ? AND codigo = ? ",
      [id_ficha, id_tipoDocumentoAdquisicion, codigo],
      (error, res) => {
        if (error) {
          reject(error);
...
userModel.postDocumentoAdquisicionDetalles = ({
  id_tipoDocumentoAdquisicion,
  fichas_id_ficha,
  codigo,
  razonSocial,
...
userModel.getCantidadRecursosByDescripcion = ({
  fichas_id_ficha,
  descripcion,
}) => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT COUNT(*) total FROM componentes LEFT JOIN partidas ON partidas.componentes_id_componente = componentes.id_componente LEFT JOIN recursos ON recursos.Partidas_id_partida = partidas.id_partida WHERE componentes.fichas_id_ficha = ? AND recursos.descripcion = ?",
      [fichas_id_ficha, descripcion],
      (error, res) => {
        if (error) {
          reject(error);
        }
...
userModel.postNuevoRecursoReal = ({
  fichas_id_ficha,
  tipo,
  id_tipoDocumentoAdquisicion,
  codigo,
  ...
  return new Promise((resolve, reject) => {
    pool.query(
      "INSERT INTO recursos_ejecucionreal (fichas_id_ficha,tipo,id_tipoDocumentoAdquisicion, codigo, descripcion,unidad,cantidad ,precio,recurso_nuevo) VALUES (?,?,?,?,?,?,?,?,1) ON DUPLICATE key UPDATE fichas_id_ficha =  values(fichas_id_ficha), tipo =  values(tipo), id_tipoDocumentoAdquisicion =  values(id_tipoDocumentoAdquisicion), codigo =  values(codigo), descripcion =  values(descripcion), unidad =  values(unidad), cantidad =  values(cantidad), precio =  values(precio) ",
      [
        fichas_id_ficha,
        tipo,
        id_tipoDocumentoAdquisicion,

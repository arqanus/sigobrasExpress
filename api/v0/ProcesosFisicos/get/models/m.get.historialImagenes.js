...
userModel.getImagenesHistorialActividades = (id_partida, callback) => {
  pool.getConnection(function (err, conn) {
    if (err) {
      ...
      conn.query(
        "SELECT DATE_FORMAT(avanceactividades.fecha, '%d-%m-%Y') fecha, avanceactividades.imagen, avanceactividades.imagenAlt, avanceactividades.descripcion FROM actividades LEFT JOIN avanceactividades ON avanceactividades.Actividades_id_actividad = actividades.id_actividad WHERE avanceactividades.imagen IS NOT NULL and actividades.Partidas_id_partida = ?",
        id_partida,
        (error, res) => {
          if (error) {
            callback(error);
...
userModel.getImagenesHistorialPartidas = (id_partida, callback) => {
  pool.getConnection(function (err, conn) {
    if (err) {
      ...
      conn.query(
        "SELECT DATE_FORMAT(partidasimagenes.fecha, '%d-%m-%Y') fecha, partidasimagenes.imagen, partidasimagenes.imagenAlt, partidasimagenes.descripcionObservacion descripcion FROM partidasimagenes WHERE partidasimagenes.imagen IS NOT NULL AND partidasimagenes.Partidas_id_partida = ?",
        id_partida,
        (error, res) => {
          if (error) {
            callback(error);
...
userModel.getPrimeraUltimaImagen = (id_partida, orden = "asc") => {
  return new Promise((resolve, reject) => {
    pool.query(
      "SELECT * FROM ((SELECT imagen, fecha, Partidas_id_partida FROM partidasimagenes) UNION (SELECT imagen, fecha, Partidas_id_partida FROM actividades LEFT JOIN avanceactividades ON avanceactividades.Actividades_id_actividad = actividades.id_actividad WHERE avanceactividades.imagen IS NOT NULL)) tb_imagenes where Partidas_id_partida = ? order by fecha " +
        orden,
      [id_partida],
      (error, res) => {
        if (error) {

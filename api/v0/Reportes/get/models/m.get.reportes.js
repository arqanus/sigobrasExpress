...
userModel.resumenValorizacionPrincipal = (
  id_ficha,
  fecha_inicial,
  fecha_final,
  costosIndirectos
      ...
      (err, res) => {
        if (err) {
          ...
          for (let i = 0; i < res.length; i++) {
            ...
            fila.porcentaje_anterior = tools.formatoPorcentaje(
              fila.porcentaje_anterior
            );
            fila.actual = tools.formatoSoles(fila.actual);
            fila.porcentaje_actual = tools.formatoPorcentaje(
              fila.porcentaje_actual
            );
            fila.acumulado = tools.formatoSoles(fila.acumulado);
            fila.porcentaje_acumulado = tools.formatoPorcentaje(
              fila.porcentaje_acumulado
            );
            fila.saldo = tools.formatoSoles(fila.saldo);
            fila.porcentaje_saldo = tools.formatoPorcentaje(
          ...
          for (let i = 0; i < costosIndirectos.length; i++) {
            ...
            fila.presupuesto = tools.formatoSoles(fila.presupuesto);
            fila.anterior = tools.formatoSoles(fila.anterior);
            fila.porcentaje_anterior = tools.tools.formatoPorcentaje(
              fila.porcentaje_anterior
            );
            fila.actual = tools.formatoSoles(fila.actual);
            fila.porcentaje_actual = tools.tools.formatoPorcentaje(
              fila.porcentaje_actual
            );
            fila.acumulado = tools.formatoSoles(fila.acumulado);
            fila.porcentaje_acumulado = tools.tools.formatoPorcentaje(
              fila.porcentaje_acumulado
            );
            fila.saldo = tools.formatoSoles(fila.saldo);
            fila.porcentaje_saldo = tools.tools.formatoPorcentaje(
              fila.porcentaje_saldo
            );
          }
            ...
            ejecutadoTotalExpediente: [
              {
                ...
                porcentaje_actual: tools.formatoPorcentaje(
                  porcentaje_actual + porcentaje_actual2
                ),
                acumulado: tools.formatoSoles(acumulado + acumulado2),
                porcentaje_acumulado: tools.formatoPorcentaje(
                  porcentaje_acumulado + porcentaje_acumulado2
                ),
                saldo: tools.formatoSoles(saldo + saldo2),
                porcentaje_saldo: tools.formatoPorcentaje(
...
userModel.resumenAvanceFisicoPartidasObraMes = (meses, id_ficha) => {
  return new Promise((resolve, reject) => {
    ...
    var query =
      "SELECT componentes.id_componente, componentes.numero, componentes.nombre, componentes.presupuesto, partidas.item, partidas.descripcion, partidas.unidad_medida, partidas.metrado,";
    ...
    query = query.concat(
      "WHERE fichas.id_ficha = ? GROUP BY partidas.id_partida ORDER BY componentes.id_componente,partidas.id_partida"
    );
      ...
      if (err) {
        ...
        for (let i = 0; i < res.length; i++) {
          ...
          if (lastIdComponente != fila.id_componente) {
            ...
            temp.push(fila.avance_acumulado, fila.saldo);
            componente.partidas = [temp];
          } else {
            var temp = [
              fila.item,
              fila.descripcion,
...
userModel.avanceMensualComparativoPresupuesto = (
  id_ficha,
  fecha_inicial,
  fecha_final
) => {
      ...
      (error, res) => {
        if (error) {
          ...
          for (let i = 0; i < res.length; i++) {
            ...
            porcentaje_programado.push(
              Number(formatoNumero(porcentaje_programado_acumulado))
            );
            if (!actual) {
              porcentaje_financiero.push(
                Number(formatoNumero(porcentaje_financiero_acumulado))
              );
              porcentaje_fisico.push(
                Number(formatoNumero(porcentaje_fisico_acumulado))
              );
            }
...
userModel.getCostoDirecto = ({ id_ficha, fecha_inicial }) => {
  return new Promise((resolve, reject) => {
    var query = `
    SELECT
...
userModel.getAvanceAnterior = ({ id_ficha, fecha_inicial }) => {
  return new Promise((resolve, reject) => {
    var query = `
    SELECT
    ...
    WHERE
        componentes.fichas_id_ficha = ${id_ficha}
            AND avanceactividades.fecha < '${fecha_inicial}'
    `;
    pool.query(query, (err, res) => {
      if (err) {
        reject(err);
...
userModel.getAvanceTotal = ({ id_ficha, fecha_final }) => {
  return new Promise((resolve, reject) => {
    var query = `
    SELECT
        SUM(avanceactividades.valor* costo_unitario)  valor

...
userModel.getmaterialesResumenEjecucionRealCodigos = (id_ficha, tipo, tipoDocumentoAdquisicion) => {
    return new Promise((resolve, reject) => {
        pool.query("SELECT codigo, COUNT(descripcion) cantidad,bloqueado FROM (SELECT recursos_ejecucionreal.codigo, recursos_ejecucionreal.descripcion,IF(recursos_ejecucionreal.documentosAdquisicion_id_documentoAdquisicion IS NULL, 0, 1) bloqueado FROM componentes LEFT JOIN partidas ON partidas.componentes_id_componente = componentes.id_componente INNER JOIN recursos ON recursos.partidas_id_partida = partidas.id_partida INNER JOIN recursos_ejecucionreal ON recursos_ejecucionreal.descripcion = CONVERT( recursos.descripcion USING UTF8) COLLATE utf8_spanish_ci WHERE componentes.fichas_id_ficha = ? AND recursos.tipo = ? AND recursos_ejecucionreal.tipoDocumentoAdquisicion_id_tipoDocumentoAdquisicion = ? AND recursos_ejecucionreal.codigo IS NOT NULL GROUP BY recursos_ejecucionreal.descripcion) recursos_ejecucionreal GROUP BY codigo", [id_ficha, tipo, tipoDocumentoAdquisicion], (error, res) => {
            if (error) {
                reject(error);
            }
            else {
...
userModel.getdocumentosadquisicion = (id_documentoAdquisicion) => {
    return new Promise((resolve, reject) => {
        pool.query("SELECT documentosadquisicion.*,clasificadores_presupuestarios.clasificador FROM documentosadquisicion left join clasificadores_presupuestarios on clasificadores_presupuestarios.id_clasificador_presupuestario = documentosadquisicion.clasificadores_presupuestarios_id_clasificador_presupuestario WHERE documentosadquisicion.id_documentoAdquisicion = ?", [id_documentoAdquisicion], (error, res) => {
            if (error) {
                reject(error);
            }
            else if (res.length == 0) {

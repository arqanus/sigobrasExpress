...
userModel.getmaterialesResumenEjecucionReal = (id_ficha, tipo) => {
    return new Promise((resolve, reject) => {
        pool.query("/*COALESCE(parcial_negativo / parcial_positivo, 0) + 1 porcentaje_negatividad_ajustado*/ SELECT coalesce( recursos_ejecucionreal.codigo,'') recurso_codigo, coalesce(tipodocumentoadquisicion.id_tipoDocumentoAdquisicion,'')id_tipoDocumentoAdquisicion,coalesce(tipodocumentoadquisicion.nombre,'')tipodocumentoadquisicion_nombre, recursos.descripcion, recursos.unidad, SUM(IF(recursos.unidad = '%MO', 0, recursos.cantidad * partidas.metrado)) recurso_cantidad, IF(recursos.unidad = '%MO', 0, recursos.precio) recurso_precio, SUM(IF(recursos.unidad = '%MO', recursos.cantidad * recursos.precio / 100, recursos.cantidad * partidas.metrado * recursos.precio)) recurso_parcial, COALESCE(recursos_ejecucionreal.cantidad, SUM(IF(recursos.unidad = '%MO', 0, partidas_metrado.avance * recursos.cantidad))) recurso_gasto_cantidad, COALESCE(recursos_ejecucionreal.precio, recursos.precio) recurso_gasto_precio, COALESCE(recursos_ejecucionreal.cantidad, SUM(IF(recursos.unidad = '%MO', 0, partidas_metrado.avance * recursos.cantidad))) * COALESCE(recursos_ejecucionreal.precio, recursos.precio) recurso_gasto_parcial, SUM(IF(recursos.unidad = '%MO', recursos.cantidad * recursos.precio / 100, recursos.cantidad * partidas.metrado * recursos.precio)) - (COALESCE(recursos_ejecucionreal.cantidad, SUM(IF(recursos.unidad = '%MO', 0, partidas_metrado.avance * recursos.cantidad))) * COALESCE(recursos_ejecucionreal.precio, recursos.precio)) diferencia, 0 porcentaje FROM componentes LEFT JOIN partidas ON partidas.componentes_id_componente = componentes.id_componente INNER JOIN recursos ON recursos.partidas_id_partida = partidas.id_partida LEFT JOIN (SELECT partidas.id_partida, COALESCE(SUM(recursos.cantidad * partidas_metrado.avance * recursos.precio), 0) ManoDeObra, partidas_metrado.avance FROM partidas LEFT JOIN recursos ON recursos.partidas_id_partida = partidas.id_partida AND recursos.tipo = 'Mano de Obra' LEFT JOIN (SELECT partidas.id_partida, COALESCE(SUM(avanceactividades.valor), 0) avance FROM componentes LEFT JOIN partidas ON partidas.componentes_id_componente = componentes.id_componente LEFT JOIN actividades ON actividades.Partidas_id_partida = partidas.id_partida INNER JOIN avanceactividades ON avanceactividades.Actividades_id_actividad = actividades.id_actividad LEFT JOIN historialactividades ON historialactividades.actividades_id_actividad = actividades.id_actividad WHERE historialactividades.estado IS NULL GROUP BY partidas.id_partida) partidas_metrado ON partidas_metrado.id_partida = partidas.id_partida GROUP BY partidas.id_partida) partidas_metrado ON partidas_metrado.id_partida = partidas.id_partida LEFT JOIN recursos_ejecucionreal ON recursos_ejecucionreal.descripcion = CONVERT( recursos.descripcion USING UTF8) COLLATE utf8_spanish_ci left join tipodocumentoadquisicion on tipodocumentoadquisicion.id_tipoDocumentoAdquisicion = recursos_ejecucionreal.tipoDocumentoAdquisicion_id_tipoDocumentoAdquisicion WHERE componentes.fichas_id_ficha = ? AND recursos.tipo = ? GROUP BY recursos.descripcion",[id_ficha, tipo], (error, res) => {
            if (error) {
                reject(error);
            }
            else if (res.length == 0) {

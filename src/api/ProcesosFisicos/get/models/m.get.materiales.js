...
userModel.getmaterialesResumen = (consulta,id_ficha, tipo) => {
    return new Promise((resolve, reject) => { 
        pool.query("/*COALESCE(parcial_negativo / parcial_positivo, 0) + 1 porcentaje_negatividad_ajustado*/ SELECT recursos.descripcion, recursos.unidad, SUM(IF(recursos.descripcion = 'HERRAMIENTAS MANUALES', 0, cantidad * partidas.metrado)) recurso_cantidad, IF(recursos.descripcion = 'HERRAMIENTAS MANUALES', 0, recursos.precio) recurso_precio, SUM(IF(recursos.descripcion = 'HERRAMIENTAS MANUALES', cantidad * precio, cantidad * partidas.metrado * recursos.precio)) recurso_parcial, SUM(IF(recursos.descripcion = 'HERRAMIENTAS MANUALES', 0, partidas_metrado.avance * recursos.cantidad)) recurso_gasto_cantidad, SUM(IF(recursos.descripcion = 'HERRAMIENTAS MANUALES', partidas_metrado.ManoDeObra * recursos.cantidad, cantidad * partidas_metrado.avance * recursos.precio)) recurso_gasto_parcial, SUM(IF(recursos.descripcion = 'HERRAMIENTAS MANUALES', cantidad * precio, cantidad * partidas.metrado * recursos.precio)) - (SUM(IF(recursos.descripcion = 'HERRAMIENTAS MANUALES', partidas_metrado.ManoDeObra * recursos.cantidad, cantidad * partidas_metrado.avance * recursos.precio))) diferencia, (SUM(IF(recursos.descripcion = 'HERRAMIENTAS MANUALES', cantidad * precio, cantidad * partidas.metrado * recursos.precio)) - (SUM(IF(recursos.descripcion = 'HERRAMIENTAS MANUALES', partidas_metrado.ManoDeObra * recursos.cantidad, cantidad * partidas_metrado.avance * recursos.precio)))) / (SUM(IF(recursos.descripcion = 'HERRAMIENTAS MANUALES', cantidad * precio, cantidad * partidas.metrado * recursos.precio))) * 100 porcentaje FROM componentes LEFT JOIN partidas ON partidas.componentes_id_componente = componentes.id_componente INNER JOIN recursos ON recursos.partidas_id_partida = partidas.id_partida LEFT JOIN (SELECT partidas.id_partida, SUM(recursos.parcial) ManoDeObra, partidas_metrado.avance FROM partidas LEFT JOIN recursos ON recursos.partidas_id_partida = partidas.id_partida AND recursos.tipo = 'Mano de Obra' LEFT JOIN (SELECT partidas.id_partida, COALESCE(SUM(avanceactividades.valor), 0) avance FROM componentes LEFT JOIN partidas ON partidas.componentes_id_componente = componentes.id_componente LEFT JOIN actividades ON actividades.Partidas_id_partida = partidas.id_partida INNER JOIN avanceactividades ON avanceactividades.Actividades_id_actividad = actividades.id_actividad LEFT JOIN historialactividades ON historialactividades.actividades_id_actividad = actividades.id_actividad WHERE historialactividades.estado IS NULL GROUP BY partidas.id_partida) partidas_metrado ON partidas_metrado.id_partida = partidas.id_partida GROUP BY partidas.id_partida) partidas_metrado ON partidas_metrado.id_partida = partidas.id_partida LEFT JOIN (SELECT partidas.id_partida, SUM(actividades.parcial) parcial_positivo FROM partidas LEFT JOIN actividades ON actividades.Partidas_id_partida = partidas.id_partida WHERE actividades.parcial > 0 GROUP BY partidas.id_partida) p1 ON p1.id_partida = partidas.id_partida LEFT JOIN (SELECT partidas.id_partida, SUM(actividades.parcial) parcial_negativo FROM partidas LEFT JOIN actividades ON actividades.Partidas_id_partida = partidas.id_partida WHERE actividades.parcial < 0 GROUP BY partidas.id_partida) p2 ON p2.id_partida = partidas.id_partida WHERE "+consulta+" = ? AND recursos.tipo = ? GROUP BY recursos.descripcion", [id_ficha, tipo], (error, res) => {
            if (error) {
                reject(error);
            }
            else if (res.length == 0) {

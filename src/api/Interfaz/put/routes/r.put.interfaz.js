...
module.exports = (app)=>{
    ...
    app.put('/putUserImagen',async(req,res)=>{
        try {
            ...
            fs.rename(formFiles.files.User_imagen.path,dir+archivo_name,(err)=>{})
            var id_usuario = await User2.getIdUsuarioIdAcceso(formFiles.fields.id_acceso)

            var affectedrows = await User.putUserImagen(id_usuario,"/static/"+folder+archivo_name)
            res.json("exito")
        } catch (error) {
